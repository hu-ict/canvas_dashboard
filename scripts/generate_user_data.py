...
class UserDictAdapter:
    def __init__(self, a_course_id, a_course_code, a_course_name, a_instance_name, a_start_date, a_end_date, a_name, a_email, a_role):
        self.canvas_course_id = a_course_id
        self.course_code = a_course_code
...
def generate_user_data(course_code, instance_name):
    ...
    for student in course.students:
        user = UserDictAdapter(course_instance.canvas_course_id, course_instance.course_code, course.name, course_instance.name, course_instance.period["start_date"], course_instance.period["end_date"], student.name, student.email, "STUDENT")
        user_data.append(user)
    for teachers in course.teachers:
        user = UserDictAdapter(course_instance.canvas_course_id, course_instance.course_code, course.name, course_instance.name, course_instance.period["start_date"], course_instance.period["end_date"], teachers.name, teachers.email, "TEACHER")
        user_data.append(user)

    with open(course_instance.get_user_data_file_name(), mode="w", newline="", encoding="utf-8") as bestand:
        veldnamen = ["canvas_course_id", "course_code", "course_name", "instance_name", "start_date", "end_date", "user_name", "user_email", "user_role"]
        csv_file = csv.DictWriter(bestand, fieldnames=veldnamen)
        csv_file.writeheader()
        for user in user_data:
            csv_file.writerow(user.to_dict())
    print("GUD97 Writing", course_instance.get_user_data_file_name())
