...
def main():
    ...
    event = "env-3_dashboard_create_event"
    ...
    for course in environment.courses:
        for course_instance in course.course_instances:
            if get_date_time_obj(course_instance.period["end_date"]) > l_actual_date and course_instance.stage != "SLEEP":
                ...
                for python_script in action.run:
                    print("R3E05 - Event", action.name, "course_code", course.name, "course_instance", course_instance.name +":>", python_script)
                    if python_script == "generate_course.py":
                        ...
                        generate_user_data(course.name, course_instance.name)
                    else:
                        print("R3E07 - Script wordt niet herkend.", python_script)
