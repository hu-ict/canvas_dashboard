...
def bandwidth_builder(assignment_group, days_in_semester):
    ...
    if assignment_group.strategy == "EXPONENTIAL":
        ...
        print(f"lower_points {assignment_group.lower_points} upper_points {assignment_group.upper_points} total_points {assignment_group.total_points}")
        print(f"lower a {lower_a:8.5} b {lower_b:5.2} c {lower_c:5.2}")
        print(f"upper a {upper_a:8.5} b {upper_b:5.2} c {upper_c:5.2}")
        serie = {0: {"day": 0, "value_day": 0, "lower": 0, "upper": int(assignment_group.total_points/30)}}
        total_points = 0
        lower_y = lower_c
        upper_y = upper_c
        output = f"Day points  0;  0;  0;{lower_y:5.2f};{upper_y:5.2f}"
        ...
        for assignment in assignment_group.assignments:
            ...
            output = f"Day points {assignment.points:2};{assignment.assignment_day:3};{total_points:>3};{lower_y:5.2f};{upper_y:5.2f}"
            output = output.replace('.',',')
            print(output)
            # print(assignment.assignment_day, assignment.points, "value_day", total_points, "lower =", lower_y, "upper =", upper_y)
