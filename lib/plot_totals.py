...
from lib.config import peil_labels, hover_style, plot_path, peil_levels, score_dict
import numpy as np
...
# Create dummy data indexed by month and with multi-columns [product, revenue]
    ...
    for level in peil_levels:
        ...
        for label in peil_labels:
            ...
            y_hover.append(label+" "+score_dict[level]['voortgang']+" "+str(student_totals['peil'][label][l_perspective][level]))
        a_fig.add_trace(go.Bar(x=x_labels, y=y_counts,
                               name=score_dict[level]['voortgang'],
                               hoverinfo="text",
                               hovertext=y_hover,
                               hoverlabel=hover_style,
                               text=y_counts,
                               marker=dict(color=score_dict[level]['color'])), 2, 1)
