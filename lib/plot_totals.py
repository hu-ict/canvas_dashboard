...
from lib.file import plot_path, tennant
from lib.lib_plotly import peil_levels, hover_style, peil_labels


def plot_progress_history(a_fig, a_progress_history, a_course, a_row, a_col, a_labels_colors):
    x = []
    ...
    for level in peil_levels:
        y = []
        y_hover = []
        l_label = a_labels_colors.level_series[a_course.perspectives[a_course.progress_perspective].levels].levels[
            str(level)].label
        l_color = a_labels_colors.level_series[a_course.perspectives[a_course.progress_perspective].levels].levels[
            str(level)].color
        for day in a_progress_history.days:
...
def plot_actuals(a_fig, a_progress_history, a_course, a_row, a_col, a_labels_colors):
    for level in peil_levels:
        y_counts = []
        x_labels = []
        y_hover = []
...
def plot_peilingen(a_fig, student_totals, a_course, a_labels_colors):
    ...
    for level in peil_levels:
        ...
        for label in peil_labels:
            ...
            y_hover.append(label+" "+a_labels_colors.level_series[a_course.perspectives[a_course.progress_perspective].levels].levels[str(level)].label+" "+str(student_totals[a_course.progress_perspective][label][l_perspective][level]))
        a_fig.add_trace(go.Bar(x=x_labels, y=y_counts,
                               name=a_labels_colors.level_series[a_course.perspectives[a_course.progress_perspective].levels].levels[str(level)].label,
                               hoverinfo="text",
                               hovertext=y_hover,
                               hoverlabel=hover_style,
                               text=y_counts,
                               marker=dict(color=a_labels_colors.level_series[a_course.perspectives[a_course.progress_perspective].levels].levels[str(level)].color)), 2, 1)

    a_fig.update_yaxes(title_text="Aantal", range=[0, 100], row=2, col=1)


def plot_totals(course, student_totals, progress_history, gilde, team, a_labels_colors):
    if tennant == "inno":
        ...
        titles = ['Project', 'Finals', 'Toets', '', 'Dagelijkse voortgang', '']
        ...
        fig.update_layout(
            ...
            xaxis3_title_text='Pending',  # xaxis perspective
            yaxis_title_text='Aantal',  # yaxis perspective
            yaxis2_title_text='Aantal',  # yaxis perspective
            # yaxis9_title_text='Aantal',  # yaxis perspective
            bargap=0.2,  # gap between bars of adjacent location coordinates
            bargroupgap=0.1,  # gap between bars of the same location coordinates
            barmode='stack'
    ...
    if tennant == "inno":
        ...
        plot_peilingen(fig, student_totals, course, a_labels_colors)
        # plot_gilde(fig, gilde)
        # plot_team(fig, team)

    plot_progress_history(fig, progress_history, course, 2, 2, a_labels_colors)
