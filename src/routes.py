...
def auth():
    ...
    try:
        ...
        return redirect('/teacher_dashboard' if "teachers" in roles else '/student_dashboard') if roles else jsonify({'message': 'Unauthorized'}), 403
...
def student_dashboard():
    user_name = jwt.decode(session['token']['access_token'], options={"verify_signature": False}).get("name", "Student")
    return jsonify({'message': f'Welcome {user_name}, you are a student!'})
